...
def get_basic_sense_hat_programme(random_uuid: str) -> str:
    """
    Utility method
    """
    return os.linesep.join([
        ...
        "sh = SenseHat()",
        f"print('{random_uuid}', sh.colour.rgb)"
    ])
...
def test_executor_live_mode_should_call_underlying_libraries(tmp_path: Path, capfd):
    ...
    with main_path.open("w") as f:
        f.write(get_basic_sense_hat_programme(random_uuid))

    executor.run(ExecutionMode.LIVE, tmp_path, str(main_path))
...
def test_executor_replay_mode_should_replay_data(tmp_path: Path, capfd):
    ...
    executor.run(ExecutionMode.REPLAY, tmp_path, str(main_path))
    captured = capfd.readouterr()
    captured_out = captured.out
    lines = [line for line in captured_out.split(os.linesep) if line.startswith(random_uuid)]
    assert len(lines) == 1
...
def test_detect_mode_when_all_modules_present_should_return_Live():
    with patch("astro_pi_executor.executor.importlib.util") as mock_importlib_util:
        mock_importlib_util.find_spec.side_effect = [Mock(name=module) \
...
def test_detect_mode_when_module_missing_should_return_Replay():
    with patch("astro_pi_executor.executor.importlib.util") as mock_importlib_util:
        mock_importlib_util.find_spec.side_effect = [None \
                for _ in AstroPiExecutor._detect_execution_mode()]
        actual_mode = AstroPiExecutor._detect_execution_mode()
...
def test_setup_venv_installs_stubs_into_venv_in_replay_mode(tmp_path: Path):
    ...
    with (tmp_path / "example.py").open("w") as f:
        f.write("") # empty file
    AstroPiExecutor.run(ExecutionMode.REPLAY, tmp_path, str(main))
    ...
    site_packages_path: Path = expected_venv_path / "lib" / python_version  / "site-packages"
