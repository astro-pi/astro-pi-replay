...
class CompletedRequest:
    ...
    def release(self):
        ...
        with self.lock:
            ...
            if self.ref_count < 0:
                ...
                if (
                    self.picam2.camera
                    and self.stop_count == self.picam2.stop_count
                    and self.picam2.started
                ):
                    self.request.reuse()
    ...
    def save(self, name, file_output, format=None, exif_data=None):
        ...
        return self.picam2.helpers.save(
            self.make_image(name), self.get_metadata(), file_output, format, exif_data
        )

    def save_dng(self, file_output, name="raw"):
        """Save a DNG RAW image of the raw stream's buffer."""
        return self.picam2.helpers.save_dng(
            self.make_buffer(name), self.get_metadata(), self.config[name], file_output
        )
...
class Helpers:
    ...
    def decompress(self, array):
        ...
        words = array.view(
            np.int32
        )  # Assume all Pis are little-endian. Note signed arithmetic is used!
        words = words.reshape(
            (words.shape[0], words.shape[1] // 2, 2)
        )  # pairs of words by component
        qmode = words & 3
        pix0 = (words >> 2) & 511
