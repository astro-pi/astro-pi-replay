#!/usr/bin/python
from astro_pi_executor.sense_hat.sense_hat_public_api import SenseHatAPI, SenseHatColourSensorAPI, SenseHatStickAPI
from astro_pi_executor.types import RGBC, RGB
...
def SenseHatColourSensorAdapter(executor: AstroPiExecutor) -> SenseHatColourSensorAPI:
        ...
        class _SenseHatColourSensorAdapter(SenseHatColourSensorAPI):
            ...
            @property
            @executor.sense_hat_replay(col_names=["R", "G", "B", "C"],
                           reducer=lambda s: tuple(s[:4]))
            ...
            def colour_raw(self) -> RGBC:
                colour = self.colour
                return tuple(map(lambda x: self._scale(x), colour))
            ...
            @property
            @executor.sense_hat_replay(col_names=["R", "G", "B"],
                           reducer=lambda s: tuple(s[:3]))
            def rgb(self) -> RGB:
                return super().rgb
